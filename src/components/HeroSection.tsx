...
const HeroSection = () => {
  const cards = [
    { name: "Snapchat+", nameAr: "سناب شات بلس", color: "from-yellow-400 to-yellow-500", Icon: SnapchatIcon },
    { name: "Telegram", nameAr: "تيليجرام مميز", color: "from-sky-400 to-sky-600", Icon: TelegramIcon },
    { name: "Verified", nameAr: "توثيق فيسبوك وانستا", color: "from-blue-500 to-purple-600", Icon: MetaVerifiedIcon },
    { name: "FB Ads", nameAr: "اعلانات ممولة", color: "from-blue-600 to-blue-800", Icon: FacebookAdsIcon },
    { name: "ChatGPT+", nameAr: "شات جي بي تي بلس", color: "from-emerald-500 to-teal-600", Icon: ChatGPTIcon },
    { name: "Netflix", nameAr: "نتفلكس", color: "from-red-600 to-red-800", Icon: NetflixIcon },
    { name: "PUBG UC", nameAr: "شدات ببجي", color: "from-orange-500 to-yellow-500", Icon: PUBGIcon },
    { name: "TikTok", nameAr: "عملات تيك توك", color: "from-gray-900 to-black", Icon: TikTokIcon, textColor: "text-cyan-400" },
    { name: "Xbox", nameAr: "اشتراك إكس بوكس", color: "from-green-500 to-green-700", Icon: XboxIcon },
    { name: "Free Fire", nameAr: "جواهر فري فاير", color: "from-orange-600 to-red-600", Icon: FreeFireIcon },
  ];
  ...
  return (
    <section className="relative flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 py-12 sm:py-16 overflow-hidden">
      ...
      <div className="relative z-10 text-center w-full max-w-5xl mx-auto">
        ...
        {/* Gift Cards Grid */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-4 mb-8 sm:mb-10 lg:mb-12 px-2">
          {cards.map((card, index) => (
            ...
            >
              <div className={`w-full h-24 sm:h-28 md:h-32 rounded-xl card-shadow bg-gradient-to-br ${card.color} overflow-hidden relative transition-transform duration-300 group-hover:scale-105`}>
                {/* Icon/Logo */}
                <div className="absolute top-2 right-2 sm:top-3 sm:right-3 w-8 h-8 sm:w-10 sm:h-10 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center text-white">
                  <card.Icon />
                </div>
                
                {/* Content */}
                <div className="flex flex-col items-center justify-center h-full pt-4">
                  <span className={`text-sm sm:text-base md:text-lg font-black tracking-tight ${card.textColor || 'text-white'}`}>{card.name}</span>
                  <p className="text-[9px] sm:text-[10px] md:text-xs text-white/80 mt-1 px-1 text-center">{card.nameAr}</p>
                </div>
              </div>
            </div>
          ))}
